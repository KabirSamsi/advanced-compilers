...
*/
export const basicBlocks = (instructions, ensureEntry = true) => {
    let blocks = new Map();
    let currentBlock = [];
    let currentBlockLabel = undefined;
    let labelCounter = 0;
    const usedLabels = new Set(instructions.filter((i) => i.label).map((i) => i.label));
    function newLabel() {
        while (usedLabels.has(`lbl${labelCounter}`))
            labelCounter++;
    ...
    if (ensureEntry === false || entryBlock === undefined)
        return blocks;
    // ensure entry
    if (blocks.entries().some(([_, insns]) => insns.at(-1)?.labels?.includes(entryBlock))) {
        let i = 1;
        while (blocks.has("entry" + i))
            i += 1;
        const newEntry = "entry" + i;
        const oldBlocks = blocks;
        blocks = new Map();
        blocks.set(newEntry, []);
        for (const [k, v] of oldBlocks)
            blocks.set(k, v);
    }
    return blocks;
};
...
*/
export const generateCFG = (blocks) => {
...
export const CFGs = async (brildata) => {
    try {
        JSON.parse(brildata);
    }
    catch {
    ...
    const data = JSON.parse(brildata);
    const ret = {};
...
//# sourceMappingURL=basicBlockCFG.js.map